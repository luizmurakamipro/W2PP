...
void ProcessImple(int conn, int level, char * str)
{
	...
	if (level >= 8)
	{
		...
		if (!strcmp(cmd, "set"))
		{
			...
			else if (!strcmp(sval1, "mobdelete"))
			{
				SendRemoveMob(conn, ival2, 0, 0);
				SendClientMessage(conn, "set mobdeleted!");
			}
			...
			else if (!strcmp(sval1, "magic"))
			{
				pMob[conn].MOB.Magic = ival2;
				pMob[conn].GetCurrentScore(conn);
				SendScore(conn);
				SendEtc(conn);
				SendClientMessage(conn, "set magic");
			}
			...
			else if (!strcmp(sval1, "maxhp"))
			{
				pMob[conn].MOB.BaseScore.MaxHp = ival2;
				pMob[conn].MOB.CurrentScore.MaxHp = ival2;
				SendClientMessage(conn, "set maxhp");
			}
			...
			/*else if (!strcmp(sval1, "maxmp"))
			{
			pMob[conn].MOB.BaseScore.MaxMp = ival2;
			pMob[conn].MOB.CurrentScore.MaxMp = ival2;
			SendClientMessage(conn, "set maxmp");
			}*/
	...
	if (level >= 2)
	{
		...
		else if (!strcmp(cmd, "create"))
		{
			...
			if (ret == 1)
				sprintf(tgsf, "Mob [%s] criado com sucesso.", sval1);
	...
	if (level >= 1) ////////////////// 0ÀÌÇÏ¸é ¿©±â µé¾î¿ÀÁö¸¦ ¾Ê´Â´Ù.//////////////////////////////////////////////////////////////
	{
		...
		else if (!strcmp(cmd, "bigquiz"))
		{
			...
			SendBigMessage(conn, "<<_No._?_>>", CuboList[value].Line[0]);
		}
		else if (!strcmp(cmd, "cubo"))
		{
			char Title[96];
			sprintf(Title, "<<_No._%d_>>", CuboN.CurrentQuestion[0] + 1);
			...
			if (cCubo)
				sprintf(tmg, "Você está no local certo!");
			else
				sprintf(tmg, "Lugar errado!");
		...
		else if (!strcmp(cmd, "citizen"))
		{
			...
			char azul[128];
			sprintf(azul, "Changed citizen to %d", ival1);
		...
		/*else if (!strcmp(cmd, "ban"))
		{
			...
			sprintf(tmg, "A conta do jogador [%s] foi banido permanentemente! Motivo: %s", sval1, sval2);
			SendNotice(tmg);

			SendBan(pUser[player].AccountName);
			CloseUser(player);
			SendClientMessage(player, "Você foi banido, entre em contato com o suporte.");
		}
		else if (!strcmp(cmd, "banmac"))
		{
			SendClientMessage(conn, "Esse comando foi desativado pelo Swedka, Obrigado.");
			return;
			...
			sprintf(tmg, "O Usuario [%s] foi banido permanentemente! Motivo: %s", sval1, sval2);
			SendNotice(tmg);

			SendMacBan(&pUser[player].Mac[0], &pUser[player].Mac[1], &pUser[player].Mac[2], &pUser[player].Mac[3]);
			CloseUser(player);
			SendClientMessage(player, "Você foi banido, entre em contato com o suporte.");
			return;
		}
		else if (!strcmp(cmd, "banip"))
		{
			...
			SendIpBan(pUser[conn].IP);
			CloseUser(player);
			SendClientMessage(player, "Você foi banido, entre em contato com o suporte.");
		}
		else if (!strcmp(cmd, "banteste"))
		{
			char tmg[256], AccDir[128], Dir[128], Account[16];
			int player = GetUserByName(sval1);
			...
			//strcpy(Account, pUser[player].AccountName);
			//strlwr(Account);

			strcpy(AccDir, ACCOUNT_DIRECTORY);

			for (int i = 0; i < strlen(AccDir); i++)
			{
				if (AccDir[i] == '\\')
					AccDir[i] = '/';
			}

			sprintf(Dir, "/%c/%s.bin", pUser[player].AccountName[0], pUser[player].AccountName);
			strcat(AccDir, Dir);

			sprintf(tmg, "O Usuario [%s] foi banido permanentemente!", sval1);
			//sprintf(tmg, "%s", AccDir);
			SendNotice(tmg);
		...
		else if (!strcmp(cmd, "gerar"))
		{
			...
			else
			{
				if (sscanf_s(innerMsg, "%ld %50s", &SCoin, sval2, _countof(sval2)))
				{
					FILE* fs = NULL;
					char Diretorio[120];
					sprintf_s(Diretorio, "userdata/Seriais/%s.txt", sval2);
					...
					char test[256];
					sprintf(test, "Pincode [%s] gerado com [%ld] Donates.", sval2, SCoin);
					SendClientMessage(conn, test);
				}
			}
		}
		else if (!strcmp(cmd, "ban3"))
		{
			char tmg[256];
			int player = GetUserByName(sval1);
			...
			sprintf(tmg, "O Usuario [%s] foi banido temporariamente por 3 Horas! Motivo: %s", sval1, sval2);
			SendNotice(tmg);

			SendBan3Hours(pUser[player].IP);
			CloseUser(player);
			SendClientMessage(player, "Você foi banido, entre em contato com o suporte.");
		}*/
	}
