	...
	case WM_COMMAND:
		...
		case IDC_LNPCList:
		{
			...
			for (int i = 0; i < MAX_CARRY; i++)
			{
				...
				SetDlgItemText(hWndMain, IDC_EItemName1 + i, g_pItemList[mob->Carry[i].sIndex].Name);
			}
		} break;
		...
		case IDC_BSave:
		{
			...
			{
				STRUCT_MOB *mob = &LoadMob[i];

				if (mob->CurrentScore.Level > MAX_LEVEL)
					mob->CurrentScore.Level = MAX_LEVEL;

				if (mob->BaseScore.Level > MAX_LEVEL)
					mob->BaseScore.Level = MAX_LEVEL;
				...
				for (int j = 0; j < MAX_EQUIP; j++)
				{
					if (mob->Equip[j].sIndex == 786 || mob->Equip[j].sIndex == 1936 || mob->Equip[j].sIndex == 1937)
					{
						for (int i = 0; i < 3; i++)
						{
							if (mob->Equip[j].stEffect[i].cEffect == 43)
							{
								if (mob->Equip[j].stEffect[i].cValue > 3)
									mob->Equip[j].stEffect[i].cValue = 3;
							}
						}
					}
				}
		...
		case IDC_BClear:
		{
			...
			for (int i = 0; i < MAX_CARRY; i++)
			{
				memset(&mob->Carry[i], 0, sizeof(STRUCT_ITEM));
				sprintf(temp, "%d %d %d %d %d %d %d", mob->Carry[i].sIndex, mob->Carry[i].stEffect[0].cEffect, mob->Carry[i].stEffect[0].cValue, mob->Carry[i].stEffect[1].cEffect, mob->Carry[i].stEffect[1].cValue, mob->Carry[i].stEffect[2].cEffect, mob->Carry[i].stEffect[2].cValue);
